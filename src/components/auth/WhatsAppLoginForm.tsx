...
export default function WhatsAppLoginForm() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    if (
      formData.whatsapp_phone_number.length !==
      selectedCountry.phone_validation_length
    ) {
      setErrors({
        whatsapp_phone_number: `Phone number must be exactly ${selectedCountry.phone_validation_length} digits`,
      });
      return;
    }

    const fullPhone = `${selectedCountry.country_code}${formData.whatsapp_phone_number}`;

    setLoading(true);
    try {
      // Use Supabase's built-in WhatsApp OTP
      ...
      setAlert({
        type: "success",
        message: `OTP sent via WhatsApp to ${fullPhone}`,
      });
    } catch (err) {
      setAlert({
        type: "error",
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session) {
        ...
        try {
          await fetch("/api/auth/whatsapp/complete-profile", {
            method: "POST",
